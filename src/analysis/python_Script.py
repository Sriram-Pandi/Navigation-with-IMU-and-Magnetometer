...
a=bagreader('/content/drive/MyDrive/RSN LAB4/data_driving_giri.bag')
e = a.message_by_topic('/imu')
d=bagreader('/content/drive/MyDrive/RSN LAB4/data_driving_giri.bag')
f = d.message_by_topic('/gps')
g=bagreader('/content/drive/MyDrive/RSN LAB4/data_going_in_circles_giri.bag')
...
imucsv = pd.read_csv('/content/drive/MyDrive/RSN LAB4/data_driving_giri/imu.csv')
...
plt.grid()
plt.scatter(v_cal[:,0], v_cal[:,1],label='Corrected magnetic field', marker=".",c='r',edgecolors='b')
...
plt.show()
plt.plot(timec_imu ,mag_x,label='Magnetic field in x before correction')
print(numpy.size(timec_imu))
print(numpy.size(v_cal[:,0]))
plt.plot(timec_imu ,v_cal[:,0],label='Magnetic field in x after correction')
plt.show()
...
magyawcrcted=yaw_cal_f
plt.plot(magyawcrcted, label='corrected Magnetic_yaw')
...
# plt.plot(magyawuncrcted, label = 'yaw_raw') 
plt.legend()
...
diff = []
for i in range(17281):
  diff = np.append(diff,(leniaraccl[i+1] - leniaraccl[i])/(timex[i+1]-timex[i]))
print(diff)
print(len(diff))
...
gpscsv = pd.read_csv('/content/drive/MyDrive/RSN LAB4/data_driving_giri/gps.csv')
...
plt.plot(timex, Forward_velocity_raw)
...
plt.plot(timex[1:],Intigrated_forward_vel*10)
...
# plt.plot(timex[1:], Intigrated_forward_vel)
# plt.show()
...
from scipy.sparse.linalg.isolve.utils import zeros
...
# ac=scipy.integrate.cumtrapz(y,time1,dx=0.025,initial=0)
# plt.plot(time1,ac)
# plt.show()
# diff=[]
# for i in range(17281):
#   diff=numpy.append(diff,(x[i+1]-x[i])/(time1[i+1]-time1[i]))
# #print(diff)
############################3
accel= y
#print(len(accel1))
...
plt.show()
########################################
accX_smooth = pyasl.smooth(y, 501, 'flat')
plt.plot(timeimu,accX_smooth)
plt.show()
velX_from_smooth_accX = scipy.integrate.cumtrapz(timeimu,accX_smooth)
plt.plot(velX_from_smooth_accX)
plt.show()
######################################

accx2= y-s.mean(y)
velX_from_accX2 = scipy.integrate.cumtrapz(timeimu,accx2)
plt.plot(velX_from_accX2)
plt.show()
#################################3
accx3=accX_smooth
framelength=40
movframe=[]
for i in range(40):
  movframe=numpy.append(movframe,accx3[i])

displacement=s.mean(movframe)
for i in range(40):
  accx3[i]=accx3[i]-displacement


for i in range(40,17281):
  accx3[i]=accx3[i]-displacement

  instmean= s.mean(movframe)
  inststd=numpy.std(movframe)

  if inststd<0.001:
    displacement=displacement+instmean
    accx3[i-framelength]=numpy.zeros(framelength)

    movframe[1]=numpy.append(movframe,[])
    movframe=numpy.append(movframe,accx3)

velX_from_accx3=scipy.integrate.cumtrapz(time_imu,accx3)
# # plt.plot(accel1)
# # plt.show()
plt.plot(time_imu,velX_from_accx3)
# # accX_smooth = smoothdata(accX,'movmean',window);
# accel2=[]

# # accel2=numpy.append(accel2,(derivative(accel1,time1,dx=0.025)))
...
# for i in range(17282):
#   if(x[i]>0):
#     acc= numpy.append(acc,x[i]) 
#   else:
#     acc=numpy.append(acc,0)
...
# plt.plot(x, label='bias')
# plt.show()
# plt.plot(diff)
# plt.show()
# accc=scipy.integrate.cumtrapz(diff_a,initial=0)
# plt.plot(accc/100)
# plt.show()
# # vel=scipy.integrate.cumtrapz(acc,initial=0)
# # plt.plot(time1,vel)
# # plt.show()
...
# yaw_comp_filter = wrapToPi(yaw_filtered)

# accX_smooth = pyasl.smooth(accex,101,'flat')
# accX3 = accX_smooth

# y=imucsv['IMU.linear_acceleration.x']
# velX_from_accX3 = scipy.integrate.cumtrapz(y)
# v_North=[]
# v_East=[]
# print(len(Forward_velocity_adjusted_x))
# print(len(yaw_filtered))

# rotation = -108*(math.pi)/180
# # for i in range(17281):
# v_East = numpy.append(v_East,(Forward_velocity_adjusted_x[i]*math.cos(yaw_filtered[i]+rotation)))
# v_North = numpy.append(v_North,(-Forward_velocity_adjusted_x[i]*math.sin(yaw_filtered[i]+rotation)))
# x_East = si.cumtrapz(time_imu[1:],v_East)
# x_North = si.cumtrapz(time_imu[1:],v_North)

# plt.plot(x_North,x_East)
# plt.show()

##############################################
# rotation = -108*(math.pi)/180
# for i in range(17281):
#   v_East = numpy.append(v_East,(velX_from_accX3[i]*math.cos(yaw_filtered[i]+rotation)))
#   v_North = numpy.append(v_North,(-velX_from_accX3[i]*math.sin(yaw_filtered[i]+rotation)))
